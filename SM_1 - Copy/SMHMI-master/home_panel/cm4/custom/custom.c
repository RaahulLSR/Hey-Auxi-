...
void setup_imgs(unsigned char *base)
{
	...
	_hp_bg21_1280x720.data = (base + 0);
	_iot_page_1280x720.data = (base + 2764800);
	_ylight_alpha_254x211.data = (base + 5529600);
	_blightt_alpha_254x211.data = (base + 5690432);
	_glightt_alpha_254x211.data = (base + 5851264);
	_thermometer_alpha_100x65.data = (base + 6012096);
	_thermometer_alpha_81x65.data = (base + 6031616);
	_airquality_alpha_116x88.data = (base + 6047424);
	_lockscreen_alpha_466x474.data = (base + 6078080);
	_img_heart_alpha_100x65.data = (base + 6740736);
	_img_ekg_alpha_100x65.data = (base + 6760256);
	_icn_water_alpha_100x100.data = (base + 6779776);
	_btn1_alpha_224x99.data = (base + 6809792);
	_arrow2_alpha_100x100.data = (base + 6876352);
	_stop4_alpha_102x104.data = (base + 6906368);
	_btn1_alpha_194x91.data = (base + 6938240);
//	_kidspage_1280x720.data = (base + 6991232);
//	_bg1_1280x720.data = (base + 9756032);
//	_btn4_alpha_299x177.data = (base + 12520832);
//	_btn4_alpha_469x278.data = (base + 12679616);
//	_btn4_alpha_296x174.data = (base + 13070784);
	base = base - 6991232;
	_kidspage_1280x720.data = (base + 6991232);
	_bg1_1280x720.data = (base + 9756032);
	_btn4_alpha_299x177.data = (base + 12520832);
 ...
 */
void gui_load_screen1(int load_kids){
    // Do something with optional_param
UI_EnterScreen_Callback1(load_kids);
return;}

void print_commands(int cid){
	print_commands_fwk(cid);
return;}


char* get_temp_custom(int i){
	static char temp_char[20];  // Make this static too
	    strcpy(temp_char, get1_temperature(i));  // Copy string safely
	    return temp_char;
}
...
void redled(){
	_SetLedColor(kRGBLedColor_White);
	GPIO_PinWrite(GPIO10, 10U, 1);
}



int last_led_state1 = 0;
void userledctrl(){
	if(last_led_state1){
...
void kids_page_call(){
	Homepage_imgbtn_1_event_handler_gest();
}

void game_page_backcall(){
	Gamepage_imgbtn_5_event_handler_gest();
}


void game_page_rock_call(){
	Gamepage_imgbtn_1_event_handler_gest();
}
void game_page_paper_call(){
	Gamepage_imgbtn_2_event_handler_gest();
}
void game_page_scissor_call(){
	Gamepage_imgbtn_3_event_handler_gest();
}
void game_page_confirm_call(){
	Gamepage_imgbtn_4_event_handler_gest();
}

void game_page_call(){
	kids_page_imgbtn_1_event_handler_gest();
}

void iot_page_call(){
	Homepage_imgbtn_3_event_handler_gest();


}

void iot_page_refresh_call(){
	iotpage_imgbtn_8_event_handler_gest();
}

void health_page_refresh_call(){
	Healthpage_imgbtn_5_event_handler_gest();
}

void rover_page_call(){
	Homepage_imgbtn_2_event_handler_gest();
}

void health_page_call(){
	Homepage_imgbtn_4_event_handler_gest();
}

void black_page_call(){
	Homepage_imgbtn_5_event_handler_gest();
}
void kids_page_backcall(){
	kids_page_imgbtn_2_event_handler_gest();
}

void rhymes_page_call(){
	kids_page_imgbtn_3_event_handler_gest();
}
void rhymes_page_backcall(){
	rhymepages_imgbtn_5_event_handler_gest();
}

void iot_page_backcall(){
	iotpage_imgbtn_4_event_handler_gest();
}

void rover_page_backcall(){
	roverpage_imgbtn_5_event_handler_gest();
}

void health_page_backcall(){
	Healthpage_imgbtn_4_event_handler_gest();
}
