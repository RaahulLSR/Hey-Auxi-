...
static void _SetFaceRec1(int activate_kids_mode)
{
	 if (activate_kids_mode == 1) {
		 taskENTER_CRITICAL();
		 vTaskSuspendAll();
		 memcpy((void *)APP_LVGL_IMGS_BASE,(void *)(0x30800000 + 6991232),6079552);
		 xTaskResumeAll();
		 taskEXIT_CRITICAL();
// Do something with optional_param
	    	return;
	        }
	 if (activate_kids_mode == 2) {
	 		 taskENTER_CRITICAL();
	 		 vTaskSuspendAll();
	 		 memcpy((void *)APP_LVGL_IMGS_BASE,(void *)(0x30800000), APP_LVGL_IMGS_SIZE + APP_ICONS_SIZE);
	 		 xTaskResumeAll();
	 		 taskEXIT_CRITICAL();
	 // Do something with optional_param
	 	    	return;
...
//static int print_commands(int command_id){
//	if (command_id == 1){
//		configPRINT_STRING("##IOT2##");
//	}
//}
...
void UI_EnterScreen_Callback1(int load_kids_level1)
{
		 _SetFaceRec1(load_kids_level1);
		 return;

}
void print_commands_fwk(int cid){
	if (cid == 1) {
	    configPRINT_STRING("##IOT1##");
	} else if (cid == 2) {
	    configPRINT_STRING("##IOT2##");
	} else if (cid == 3) {
	    configPRINT_STRING("##IOT3##");
	} else if (cid == 4) {
	    configPRINT_STRING("##IOT4##");
	} else if (cid == 5) {
	    configPRINT_STRING("##IOT5##");
	} else if (cid == 6) {
	    configPRINT_STRING("##IOT6##");
	} else if (cid == 7) {
	    configPRINT_STRING("##IOT7##");
	} else if (cid == 8) {
	    configPRINT_STRING("##IOT8##");
	} else if (cid == 9) {
	    configPRINT_STRING("##info##");
	}else if (cid == 10) {
	    configPRINT_STRING("##alert##");
	}else if (cid == 11) {
	    configPRINT_STRING("##call##");
	}

}

	char* get1_temperature(int i){
		if(i == 1){
		configPRINT_STRING("##rtemp##");
		   // Parse the string for temperature and humidity
		static char uat_inp[100];
		int readChars = DbgConsole_ReadLine((uint8_t*)uat_inp, sizeof(uat_inp));
		...
		return strcat(uat_inp, "");

	}
		else if(i == 2){
			configPRINT_STRING("##humid##");
					   // Parse the string for temperature and humidity
					static char uat_inp[100];
					int readChars = DbgConsole_ReadLine((uint8_t*)uat_inp, sizeof(uat_inp));
...
void turnledcolor()
{
	APP_OutputDev_RgbLed_InferCompleteDecode();
	return;

}

void homeenterScreen_Callback()
{	screen_name = 0;
	_StopVoiceSession(kScreen_Home);
	_SetFaceRec(kScreen_Home, kOASISLiteState_Recognition);
}
void homeexitScreen_Callback()
{
	_SetFaceRec(kScreen_Home, kOASISLiteState_Stopped);
}

void rhymesenterScreen_Callback()
{	sub_screen_name = 1;

}

void rhymesexitScreen_Callback()
{	sub_screen_name = 0;


}

void gamesenterScreen_Callback()
{	sub_screen_name = 2;
_EnableGestureControl(true);

}

void gamesexitScreen_Callback()
{	sub_screen_name = 0;
_EnableGestureControl(false);

}

void IOTenterScreen_Callback()
{	screen_name = 1;
	_StopVoiceSession(kScreen_Home);
}
void iotexitScreen_Callback()
{
	_StopVoiceSession(kScreen_Home);
	homeenterScreen_Callback();
}

void healthenterScreen_Callback()
{	screen_name = 4;
	_StopVoiceSession(kScreen_Home);
}
void healthexitScreen_Callback()
{
	_StopVoiceSession(kScreen_Home);
	homeenterScreen_Callback();
}

void lockenterScreen_Callback()
{	screen_name = 5;
	_StopVoiceSession(kScreen_Home);
}
void lockexitScreen_Callback()
{
	_StopVoiceSession(kScreen_Home);
	homeenterScreen_Callback();
}

void roverenterScreen_Callback()
{	screen_name = 2;
	_StopVoiceSession(kScreen_Home);
	_EnableGestureControl(true);
}

void roverexitScreen_Callback()
{
...
void kidsenterScreen_Callback()
{	screen_name = 3;
	_StopVoiceSession(kScreen_Home);
}
void kidsexitScreen_Callback()
{
	_StopVoiceSession(kScreen_Home);
	homeenterScreen_Callback();
}
...
{	if(screen_name == 2){
   ...
   // _DrawHandMarkers(&s_UiSurface, &pHandResult->hand);
    ...
    else if (oldGestureSign != pHandResult->hand.gtype)
    {
        ...
        else if (pHandResult->hand.gtype == 6)
        {
        ...
        else if (pHandResult->hand.gtype == 2)
        {
        					GPIO_PinWrite(GPIO10,(10U) , 0U);
        					GPIO_PinWrite(GPIO10,(9U) , 1U);
        					GPIO_PinWrite(GPIO10,(12U) , 0U);
        ...
        if (voiceAlgoResult->status == ASR_CMD_DETECT && voiceAlgoResult->keywordID > -1)
        {
            ...
            switch (voiceAlgoResult->keywordID)
            {
				...
				case (7):
				{
					 ...
					 configPRINT_STRING("##IOT6##");
					 vTaskDelay(2000);
					 PlayPrompt(turning_on_lights,1);
					 vTaskDelay(1000);
					 configPRINT_STRING("##IOT1##");
					 PlayPrompt(demo4,1);
					 vTaskDelay(6000);
					 iot_page_refresh_call();
					 vTaskDelay(300);
					 PlayPrompt(turning_off_lights,1);
					 vTaskDelay(1000);
					 configPRINT_STRING("##IOT2##");
					 //age_backcall();
				...
				case (8):
				{PlayPrompt(PROMPT_BEEP,1);
					PlayPrompt(how_are_you,1);
				}
				break;
				case (9):
				{	PlayPrompt(PROMPT_BEEP,1);
					PlayPrompt(introduce_yourself,1);
				}
				break;
				case (10):
				{	PlayPrompt(PROMPT_BEEP,1);
        ...
        if (voiceAlgoResult->status == ASR_CMD_DETECT && voiceAlgoResult->keywordID > -1)
        {
            ...
            case (0):
            	...
            	game_page_rock_call();}
                            ...
                            {	if(sub_screen_name == 2){
                            	PlayPrompt(PROMPT_BEEP,1);
                            	game_page_scissor_call();}
                            }
                            break;
                            case (3):
            				{ 	if(sub_screen_name == 2){
                            	PlayPrompt(PROMPT_BEEP,1);
            				game_page_confirm_call();}
            				}
            				break;
            				case (4):
            				{PlayPrompt(PROMPT_BEEP,1);
            				...
            				if(sub_screen_name == 1){
            					rhymes_page_backcall();
                                                                   ...
                                                                   void *inferResult)
{
    ...
    //LVGL_LOCK();
    //home_panel_screen_id_t currentScreenId = get_current_screen();
    home_panel_screen_id_t currentScreenId = screen_name;
